...
Maps are **Tiled tilemaps at 64 px**, generated by an offline asset pipeline —
see [`docs/map-architecture.md`](docs/map-architecture.md) and
...
- **Tilemaps** — authored in `tools/asset-pipeline/`, emitted to `public/assets/maps/*.json`, registered in `src/game/maps/map-registry.ts`. A new location is a map definition plus one registry row — `TileMapScene` needs no changes. Towns and interiors are authored as **character grids** (`tools/asset-pipeline/mapdefs/`); large organic maps use the programmatic `MapBuilder`.
- **Town layouts (legacy)** — `data/maps/*-town-config.ts`. Only Eldric and Dreadshore still use `TownScene`; new towns should be tilemaps.
- **World triggers and zones** — object layers inside each map's Tiled JSON, not a TypeScript config.
...
Everything under `public/assets/{tilesets,atlases,sprites,maps}/` is produced by
`npm run assets` from the two source PNGs in the repository root. Change a
generator in `tools/asset-pipeline/` and re-run; never edit the outputs.

The system addresses art by **name**, never by index — so replacing artwork means
shipping a tile or atlas frame with the same name, and no map data or code
changes. Buildings, interior furnishings and character sprites are procedurally
generated placeholders precisely so they can be swapped this way.

Verify changes with `node tools/asset-pipeline/preview-maps.mjs --scale 0.3`,
which renders each map to PNG exactly as the game layers it.
